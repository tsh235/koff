  ...
  render(data) {
    ...
    if (data.images.length > 1) {
      ...
      const thumbnailsSliderItems = data.images.map(item => {
        const productThumbnailsSlide = document.createElement('div');
        productThumbnailsSlide.classList.add('swiper-slide', 'product__thumbnails-slide');
        
        const productThumbnailsImage = document.createElement('img');
        productThumbnailsImage.classList.add('product__thumbnails-img');
        productThumbnailsImage.src = `${API_URL}${item}`;
  
        productThumbnailsSlide.append(productThumbnailsImage);
    ...
    productCharacteristics.append(productCharacteristicsTitle, characteristicsTable);
